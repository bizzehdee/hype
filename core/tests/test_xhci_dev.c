...
static uint64_t push_cmd(rig_t *r, uint64_t param, uint32_t status, uint8_t type, uint8_t slot) {
    uint32_t off = OFF_CMD + (uint32_t)r->cmd_enqueue * HYPE_XHCI_TRB_SIZE;
    uint32_t control = ((uint32_t)type << HYPE_XHCI_TRB_TYPE_SHIFT) |
                       ((uint32_t)slot << HYPE_XHCI_TRB_SLOT_SHIFT) |
                       (r->cmd_pcs ? HYPE_XHCI_TRB_CYCLE : 0u);
...
static void rig_init(rig_t *r) {
    ...
    wr32(r, HYPE_XHCI_RT_ERSTSZ, 1u);
    wr32(r, HYPE_XHCI_RT_ERSTBA_LO, (uint32_t)(GBASE + OFF_ERST));
    wr32(r, HYPE_XHCI_RT_ERSTBA_HI, (uint32_t)((GBASE + OFF_ERST) >> 32));
    wr32(r, HYPE_XHCI_RT_ERDP_LO, (uint32_t)(GBASE + OFF_EVT));
    wr32(r, HYPE_XHCI_RT_ERDP_HI, (uint32_t)((GBASE + OFF_EVT) >> 32));
    wr32(r, HYPE_XHCI_RT_IMAN, HYPE_XHCI_IMAN_IE);

    /* DCBAAP + the command ring (RCS = 1). */
    put64(r, OFF_DCBAA + 8 * 1, GBASE + OFF_DEVCTX); /* DCBAA[1] -> slot 1 device context */
    wr32(r, HYPE_XHCI_OP_DCBAAP_LO, (uint32_t)(GBASE + OFF_DCBAA));
    wr32(r, HYPE_XHCI_OP_DCBAAP_HI, (uint32_t)((GBASE + OFF_DCBAA) >> 32));
    wr32(r, HYPE_XHCI_OP_CRCR_LO, (uint32_t)(GBASE + OFF_CMD) | HYPE_XHCI_CRCR_RCS);
    wr32(r, HYPE_XHCI_OP_CRCR_HI, (uint32_t)((GBASE + OFF_CMD) >> 32));
    wr32(r, HYPE_XHCI_OP_USBCMD, HYPE_XHCI_USBCMD_RS | HYPE_XHCI_USBCMD_INTE);
}
...
static void read_event(rig_t *r, unsigned n, uint64_t *param, uint32_t *status, uint32_t *control) {
    uint32_t off = OFF_EVT + n * HYPE_XHCI_TRB_SIZE;
...
static void test_capability_registers(void) {
    rig_t r;
    rig_init(&r);
    CHECK_HEX("CAPLENGTH", HYPE_XHCI_CAPLENGTH, rd32(&r, HYPE_XHCI_CAP_CAPLENGTH) & 0xFFu);
    CHECK_HEX("HCIVERSION", HYPE_XHCI_HCIVERSION, (rd32(&r, HYPE_XHCI_CAP_CAPLENGTH) >> 16) & 0xFFFFu);
    CHECK_HEX("HCSPARAMS1", HYPE_XHCI_HCSPARAMS1, rd32(&r, HYPE_XHCI_CAP_HCSPARAMS1));
    CHECK_HEX("DBOFF", HYPE_XHCI_DBOFF, rd32(&r, HYPE_XHCI_CAP_DBOFF));
    CHECK_HEX("RTSOFF", HYPE_XHCI_RTSOFF, rd32(&r, HYPE_XHCI_CAP_RTSOFF));
    /* MaxPorts and MaxSlots fields. */
    CHECK_INT("MaxSlots", HYPE_XHCI_MAX_SLOTS, rd32(&r, HYPE_XHCI_CAP_HCSPARAMS1) & 0xFFu);
    CHECK_INT("MaxPorts", HYPE_XHCI_MAX_PORTS, (rd32(&r, HYPE_XHCI_CAP_HCSPARAMS1) >> 24) & 0xFFu);
    /* Capability registers are read-only. */
    wr32(&r, HYPE_XHCI_CAP_HCSPARAMS1, 0xDEADBEEFu);
    CHECK_HEX("HCSPARAMS1 stays RO", HYPE_XHCI_HCSPARAMS1, rd32(&r, HYPE_XHCI_CAP_HCSPARAMS1));
}
...
static void test_port_reset(void) {
    ...
    uint32_t psc;
    rig_init(&r);
    psc = rd32(&r, HYPE_XHCI_OP_PORTSC);
    CHECK_INT("port connected at reset", 1, (psc & HYPE_XHCI_PORTSC_CCS) ? 1 : 0);
    CHECK_INT("connect change set", 1, (psc & HYPE_XHCI_PORTSC_CSC) ? 1 : 0);
    CHECK_INT("not yet enabled", 0, (psc & HYPE_XHCI_PORTSC_PED) ? 1 : 0);
    /* Ack the connect change (RW1C) and issue a port reset. */
    wr32(&r, HYPE_XHCI_OP_PORTSC, HYPE_XHCI_PORTSC_CSC | HYPE_XHCI_PORTSC_PR);
    psc = rd32(&r, HYPE_XHCI_OP_PORTSC);
    CHECK_INT("connect change cleared", 0, (psc & HYPE_XHCI_PORTSC_CSC) ? 1 : 0);
    CHECK_INT("enabled after reset", 1, (psc & HYPE_XHCI_PORTSC_PED) ? 1 : 0);
    CHECK_INT("reset change set", 1, (psc & HYPE_XHCI_PORTSC_PRC) ? 1 : 0);
    CHECK_INT("PR self-cleared", 0, (psc & HYPE_XHCI_PORTSC_PR) ? 1 : 0);
}
...
static void test_enable_slot(void) {
    ...
    CHECK_INT("event type = cmd completion", HYPE_XHCI_TRB_CMD_COMPLETION,
              (control >> HYPE_XHCI_TRB_TYPE_SHIFT) & 0x3Fu);
    CHECK_INT("completion = success", HYPE_XHCI_CC_SUCCESS, (status >> 24) & 0xFFu);
    CHECK_INT("slot 1 allocated", 1, (control >> HYPE_XHCI_TRB_SLOT_SHIFT) & 0xFFu);
    CHECK_INT("event cycle = 1 (first pass)", 1, control & HYPE_XHCI_TRB_CYCLE);
    CHECK_HEX("event param = command TRB gpa", GBASE + OFF_CMD, param);
    CHECK_INT("slot state enabled", HYPE_XHCI_SLOT_ENABLED, r.dev.slots[1].state);
...
static void test_bringup_sequence(void) {
    ...
    CHECK_INT("address device success", HYPE_XHCI_CC_SUCCESS, (status >> 24) & 0xFFu);
    CHECK_INT("slot addressed", HYPE_XHCI_SLOT_ADDRESSED, r.dev.slots[1].state);
    /* The output device context slot-context state field must read Addressed, address = slot 1. */
    CHECK_INT("devctx slot state", HYPE_XHCI_SLOT_ADDRESSED, (get32(&r, OFF_DEVCTX + 12u) >> 27) & 0x1Fu);
    ...
    CHECK_INT("slot configured", HYPE_XHCI_SLOT_CONFIGURED, r.dev.slots[1].state);
...
static void test_link_trb_and_noop(void) {
    ...
    push_cmd(&r, 0, 0, HYPE_XHCI_TRB_NOOP_CMD, 0); /* TRB 0 */
    /* TRB 1 = Link -> TRB 2 (GBASE+OFF_CMD+2*16), no toggle. Build it directly (not a command). */
    {
        uint32_t off = OFF_CMD + 1u * HYPE_XHCI_TRB_SIZE;
        put64(&r, off, GBASE + OFF_CMD + 2u * HYPE_XHCI_TRB_SIZE);
        put32(&r, off + 8, 0);
        put32(&r, off + 12,
              ((uint32_t)HYPE_XHCI_TRB_LINK << HYPE_XHCI_TRB_TYPE_SHIFT) | HYPE_XHCI_TRB_CYCLE);
    ...
    read_event(&r, 1, &param, &status, &control);
    CHECK_INT("second no-op success", HYPE_XHCI_CC_SUCCESS, (status >> 24) & 0xFFu);
    CHECK_HEX("second no-op TRB gpa (past the link)", GBASE + OFF_CMD + 2u * HYPE_XHCI_TRB_SIZE,
              param);
}
...
static void test_hcreset_clears_state(void) {
    ...
    CHECK_INT("halted after reset", 1, (rd32(&r, HYPE_XHCI_OP_USBSTS) & HYPE_XHCI_USBSTS_HCH) ? 1 : 0);
    CHECK_INT("port still connected", 1, (rd32(&r, HYPE_XHCI_OP_PORTSC) & HYPE_XHCI_PORTSC_CCS) ? 1 : 0);
}
...
static void test_mmio_access_widths(void) {
    ...
    CHECK_INT("8-byte read rc", 0, hype_xhci_dev_mmio_read(&r.dev, HYPE_XHCI_OP_DCBAAP_LO, 8u, &v));
    CHECK_HEX("DCBAAP 64-bit", GBASE + OFF_DCBAA, v);
    /* Byte read of CAPLENGTH. */
    CHECK_INT("byte read rc", 0, hype_xhci_dev_mmio_read(&r.dev, HYPE_XHCI_CAP_CAPLENGTH, 1u, &v));
    CHECK_HEX("CAPLENGTH byte", HYPE_XHCI_CAPLENGTH, v);
    /* Word read of HCIVERSION (offset 0x02). */
    CHECK_INT("word read rc", 0, hype_xhci_dev_mmio_read(&r.dev, HYPE_XHCI_CAP_HCIVERSION, 2u, &v));
    CHECK_HEX("HCIVERSION word", HYPE_XHCI_HCIVERSION, v);
    ...
    CHECK_INT("past BAR rejected", -1,
              hype_xhci_dev_mmio_read(&r.dev, HYPE_XHCI_BAR_SIZE, 4u, &v));
    /* 8-byte write of ERSTBA then read back. */
    CHECK_INT("8-byte write rc", 0,
              hype_xhci_dev_mmio_write(&r.dev, HYPE_XHCI_RT_ERSTBA_LO, 8u,
                                       (GBASE + OFF_ERST) | 0u, &r.map));
    CHECK_HEX("ERSTBA lo readback", (uint32_t)(GBASE + OFF_ERST), rd32(&r, HYPE_XHCI_RT_ERSTBA_LO));
}
...
static void test_usbsts_and_iman_rw1c(void) {
    ...
    wr32(&r, HYPE_XHCI_OP_USBSTS, HYPE_XHCI_USBSTS_EINT);
    wr32(&r, HYPE_XHCI_RT_IMAN, HYPE_XHCI_IMAN_IP | HYPE_XHCI_IMAN_IE);
    CHECK_INT("EINT cleared", 0, (rd32(&r, HYPE_XHCI_OP_USBSTS) & HYPE_XHCI_USBSTS_EINT) ? 1 : 0);
    CHECK_INT("IP cleared", 0, (rd32(&r, HYPE_XHCI_RT_IMAN) & HYPE_XHCI_IMAN_IP) ? 1 : 0);
...
static void test_event_ring_wrap(void) {
    ...
    wr32(&r, HYPE_XHCI_RT_ERSTSZ, 1u);
    wr32(&r, HYPE_XHCI_RT_ERSTBA_LO, (uint32_t)(GBASE + OFF_ERST)); /* re-latch */
    ...
    read_event(&r, 0, &param, &status, &control);
    CHECK_INT("wrapped event cycle = 0", 0, control & HYPE_XHCI_TRB_CYCLE);
