...
static void compress(hype_sha256_t *s, const uint8_t *p) {
...
void hype_sha256_update(hype_sha256_t *s, const void *data, unsigned long len) {
    ...
    s->total_len += len;
    while (len > 0ul) {
        unsigned n = 64u - s->block_len;
        unsigned long i;
        if ((unsigned long)n > len) n = (unsigned)len;
        for (i = 0; i < n; i++) s->block[s->block_len + i] = p[i];
        s->block_len += n;
        p += n;
        len -= n;
        if (s->block_len == 64u) {
...
void hype_sha256_final(hype_sha256_t *s, uint8_t digest[HYPE_SHA256_DIGEST_SIZE]) {
    uint64_t bits = s->total_len * 8u;
    unsigned i;
    uint8_t pad = 0x80;
    hype_sha256_update(s, &pad, 1);
    pad = 0;
    while (s->block_len != 56u) hype_sha256_update(s, &pad, 1);
    for (i = 0; i < 8u; i++) {
        uint8_t b = (uint8_t)(bits >> (56u - 8u * i));
        /* length must not re-count itself */
        s->block[s->block_len++] = b;
    }
    compress(s, s->block);
    for (i = 0; i < 8u; i++) {
...
void hype_sha256(const void *data, unsigned long len, uint8_t digest[HYPE_SHA256_DIGEST_SIZE]) {
    hype_sha256_t s;
    hype_sha256_init(&s);
    hype_sha256_update(&s, data, len);
    hype_sha256_final(&s, digest);
}
