...
#endif
/* Non-static (#694): shared with arch/x86_64/vmm_device_ops.c's
 * process_ahci_command_slot(), which gates the same ABAR MMIO trace. */
int g_ahci_trace = 0;
...
/* #343 ATAPI transfer-accounting counters, and hype_svm_vcpu_get_atapi_diag()/
 * hype_svm_vcpu_get_read_verify() that read them, moved to
...
/* hype_guest_dma_xlate moved to core/guest_mem.c as hype_guest_dma_xlate() (#693/#694) --
 * shared with the extracted virtio-blk/AHCI modules and with arch/x86_64/vmx/vmcs_hw.c. */

/* ahci_copy_fast/complete_ahci_soft_reset/ahci_bus_master_refused/process_ahci_command_slot
 * moved to arch/x86_64/vmm_device_ops.c (#694) -- vendor-neutral, shared by both backends.
 * g_ahci_trace below stays here (non-static) since hype_svm_ahci_atapi_npf_common() -- which
 * stays in this file -- gates its own trace on the same flag. */
...
static int hype_svm_ahci_atapi_npf_common(struct hype_vcpu_ctx *real, hype_ahci_t *ahci,
    ...
    if (guest_insn_bytes != 0) {
        ...
        guest_bytes = (const uint8_t *)(uintptr_t)hype_guest_dma_xlate(dma_map, real->vmcb->save.rip, 1u);
        if (guest_bytes == 0) {
            return -1;
        }
    }
...
/* complete_ahci_command_slot/ahci_backend_rw_prdt/process_ahci_ata_command_slot moved to
 * arch/x86_64/vmm_device_ops.c (#694) -- vendor-neutral, shared by both backends. */
...
static int hype_svm_ahci_disk_npf_common(hype_vcpu_ctx_t *ctx, hype_ahci_t *ahci,
    ...
    guest_bytes = (guest_insn_bytes != 0)
                      ? guest_insn_bytes
                      : (const uint8_t *)(uintptr_t)hype_guest_dma_xlate(
 ...
 * hype_guest_dma_xlate does only guest-physical -> host, which is enough for an identity-mapped
 ...
 * each table page is read host-side via hype_guest_dma_xlate. Returns the guest-physical address, or
...
static uint64_t guest_linear_to_phys(const hype_gpa_map_t *dma_map, uint64_t cr3, uint64_t laddr) {
    ...
    for (level = 0; level < 4; level++) {
        uint64_t host = hype_guest_dma_xlate(dma_map, table, 4096u);
        uint64_t e;
        unsigned idx;
        if (host == 0) {
            return ~0ull;
     ...
     * the CRB side through the model and the RAM side through the NPT translation. */
    {
        ...
        if (guest_insn_bytes != 0 &&
            ...
            while (count > 0u) {
                ...
                ram_host = hype_guest_dma_xlate(dma_map, ram_phys, elem);
                if (ram_host == 0) {
                    return -1;
                }
                if (to_mmio) {
...
/* virtq_validate_chain/process_virtio_blk_queue moved to arch/x86_64/vmm_device_ops.c (#693) --
 * vendor-neutral, shared by both backends. hype_virtio_blk_set_reject_sink() moved with them. */
