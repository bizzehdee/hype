#!/bin/bash
# #746: a keyboard BEHIND A HUB, plugged in after boot, must work -- with no reboot.
#
...
#
# This is the operator's own topology. #744 and #745 cover root ports, where the xHCI
# raises a Port Status Change Event; a hub's downstream ports raise NOTHING on the event
# ring. The hub reports them on its own status-change interrupt endpoint, which hype never
# configured -- hype_xhci_configure_hub_slot() still sets Context Entries to 1 for exactly
# that reason -- so before #746 the keyboard on the 5950X's 2.0 hub could not hot-plug at
# all, and the root-port rigs would have said everything was fine.
#
# The cycle, run BEHIND THE HUB:
#
...
(
  ...
  printf 'device_add usb-kbd,id=kbd0,bus=xhci.0,port=2.2\n'
  sleep 20
  ...
  -device usb-kbd,id=kbd0,bus=xhci.0,port=2.2 \
...
echo "=== the hub's status endpoint"
grep -a "status-change endpoint" $S/serial.log | head -6
echo "=== the departure and arrival"
grep -a "hub slot .* port .* changed\|DEPARTED\|hub slot .* ARRIVED\|Disable Slot" $S/serial.log | head -10
...
if [ "${hr:-0}" -gt 0 ] 2>/dev/null; then
  grep -aq "host-hid: keyboard .* DEPARTED" $S/serial.log ||
    say "FAIL: the hub reported but the departure was not acted on [#746]"
fi

# #745's own bar.
# What this rig CAN check: that hype arms the endpoint and polls it cleanly.
grep -aq "hub slot .* status-change endpoint .* armed" $S/serial.log ||
  say "FAIL: no hub status-change endpoint was armed -- nothing can see behind a hub [#746]"
hubline=$(grep -a "DIAG: HUBPOLL" $S/serial.log | tail -1)
echo "$hubline"
hp=$(echo "$hubline" | sed -n 's/.*polls=\([0-9]*\).*/\1/p')
hr=$(echo "$hubline" | sed -n 's/.*reports=\([0-9]*\).*/\1/p')
he=$(echo "$hubline" | sed -n 's/.*errors=\([0-9]*\).*/\1/p')
[ "${hp:-0}" -gt 100 ] 2>/dev/null ||
  say "FAIL: hype barely polled the hub status endpoint (polls=${hp:-0}) [#746]"
...
echo "FIRST-report lines: $nfirst (one per claim)"
if [ "${hr:-0}" -gt 0 ] 2>/dev/null; then
  [ "$nfirst" -ge 2 ] || say "FAIL: the re-plugged keyboard never reported [#746]"
fi
