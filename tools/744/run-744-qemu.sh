#!/bin/bash
# #744: hype must NOTICE a device leaving a root port, and tear its slot down.
#
...
#
# The keyboard here is on a ROOT PORT, which is this ticket's scope. A device behind a hub
# is #746: unplugging it leaves the root port connected, because the hub is still there,
# so none of this fires. The mouse stays behind the hub as the control -- it must keep
# working across the keyboard's departure.
#
# The unplug is `device_del` on the QEMU monitor, which is a real USB disconnect: the
# controller raises a Port Status Change Event exactly as hardware does.
set -e
...
# #744's bar, in order of what it proves.
grep -aq "host-usb: port .* changed -- now empty" $S/serial.log ||
  say "FAIL: no Port Status Change Event was noticed at all [#744]"
grep -aq "host-hid: keyboard .* DEPARTED" $S/serial.log ||
  say "FAIL: the departure was seen but the claimed keyboard was not released [#744]"
grep -aq "host-xhci:     Disable Slot:" $S/serial.log ||
  say "FAIL: no Disable Slot was issued -- the slot leaked [#744]"
...
[ $fail -eq 0 ] && echo "ALL PASS: the departure was noticed, the slot released, the mouse unaffected"
exit $fail
