#!/bin/bash
# #698 boot-A shape: hype2g.cfg under nested SVM -- the Alpine reboot-pin guest plus the three
# #603 microtests, 7 vCPU cores, to reproduce the vm0 housekeeping stall seen on Intel boot A.
# Derived from tools/525/run-525.sh.
...
firmware = uefi
os_hint = linux
target_disk = file:\hype\disks\run2b.img
[vm.vmexit]
label = micro/vmexit
vcpus = 2
mem_mb = 512
...
mcopy -i "$E" tools/hw-val-2026-08-25/input-2b/vm0.txt ::/input/vm0.txt
mmd -i "$E" ::/EFI/hype/micro
mcopy -i "$E" build/micro/vmexit.bin build/micro/vmexitstorm.bin build/micro/hello.bin ::/EFI/hype/micro/

for ATTEMPT in 1 2 3; do
  cp "$VARS" $S/VARS.fd
  timeout "$SECS" "$QEMU" \
    -machine q35 -m 6144 -nodefaults \
    -accel kvm -accel tcg -cpu host -smp 10 \
...
sync
if grep -aq "reboot-pin-nonbsp" $S/serial.txt; then
